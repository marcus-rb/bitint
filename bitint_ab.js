...
const SHA_2 = (message, digestSize, charsetSize = 8) => {
  ...
  let preProcessedMessage = charsetSize === 8 ?
                              new Uint8Array(initialMessageLength) :
                            charsetSize === 16 ?
                              new Uint16Array(initialMessageLength) :
                              new Uint32Array(initialMessageLength) ;
  ...
  const postProcessedMessageTargetLength =
    (Math.floor(initialMessageLength / 512) + 1) * 512 ;

  const bitsToAdd = postProcessedMessageTargetLength - 65 - initialMessageLength;
  //console.log("Target length: "+postProcessedMessageTargetLength);
  //console.log(bitsToAdd % charsetSize);

  console.timeEnd("Hash completed in");
  //console.log(preProcessedMessageTargetLength);
  //console.log(hashConstantsLocal);
  console.log(roundConstantsLocal);
  return;
}
